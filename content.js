...
(function() {
  ...
  settingsPanel.innerHTML = `
    ...
    <div id="waqr-change-email-box" style="display:none;">
      <input type="email" id="waqr-change-new" placeholder="New email address" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:6px;margin-bottom:8px;box-sizing:border-box;font-size:13px;">
  ...
  if (changeToggle && changeBox) {
    ...
    changeSave.addEventListener('click', async () => {
      ...
      console.log('[AUDIT][EXTENSION][EMAIL_CHANGE] New email:', nw);
      changeErr.style.display = 'none';
      const valid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!valid.test(nw)) {
        console.log('[AUDIT][EXTENSION][EMAIL_CHANGE] Invalid email format');
        changeErr.textContent = 'Please enter a valid email address'; changeErr.style.display = 'block'; return;
      }
      ...
      if (!token) {
        ...
        changeErr.textContent = 'Please reconnect the extension to change your email. Open extension options and click "Connect" with your handshake token.';
